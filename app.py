...
def flow_submission_webhook():
    ...
    try:
        ...
        # Get contact_id from the flow_token (format: booking_{contact_id}_{timestamp})
        if flow_token and flow_token.startswith('booking_'):
            parts = flow_token.split('_')
            if len(parts) >= 2:
                contact_id_from_token = parts[1]
                # Try to get contact_id from token if not provided
                if not payload.get('contact_id'):
                    # You'll need to map token to contact_id or extract from token
                    pass
        ...
        print(f"   Contact ID from payload: {contact_id}")
        ...
        parking_response = flow_data.get('screen_0_Will_you_need_Parking_Space_3', 'No')
        parking_needed = parking_response == 'Yes'
        
        print(f"   Booking Date: {booking_date}")
        print(f"   Adults: {adults}")
        print(f"   Children: {children}")
        print(f"   Parking Needed: {parking_needed}")
        
        # Screen 1 data (activity counts)
        activities = {
            'zipline': int(flow_data.get('screen_1_Ziplining_0', 0) or 0),
            'horse_riding': int(flow_data.get('screen_1_Horse_Riding_1', 0) or 0),
            'vr': int(flow_data.get('screen_1_VR_VIRTUAL_REALITY_2', 0) or 0),
            'giant_swing': int(flow_data.get('screen_1_Giant_Swing_3', 0) or 0),
            'boat_cruise': int(flow_data.get('screen_1_Boat_Cruise_4', 0) or 0),
            'canoeing': int(flow_data.get('screen_1_Canoeing_5', 0) or 0),
            'fishing': int(flow_data.get('screen_1_Fishing_6', 0) or 0)
        }
        ...
        print(f"   Costs calculated: Entrance: ${costs['entrance_fee']}, Activities: ${costs['activities_total']}, Total: ${costs['total']}")
        
        # Generate booking reference
        booking_ref = create_booking_reference()
        
        # Get contact info - try to find by phone number or flow_token
        with get_db_cursor(commit=False) as cursor:
            if contact_id:
                cursor.execute('SELECT id, phone_number, display_name FROM contacts WHERE id = %s', (contact_id,))
                contact = cursor.fetchone()
            else:
                # Try to find contact by phone number in flow data or use a default
                # This is a fallback - you should store contact_id in flow_token
                cursor.execute('SELECT id, phone_number, display_name FROM contacts ORDER BY id DESC LIMIT 1')
                contact = cursor.fetchone()
            
            if not contact:
                # Create a temporary contact record
                contact = {'id': None, 'phone_number': None, 'display_name': 'Guest'}
        ...
        if contact and contact.get('id'):
            save_activity_booking(contact['id'], booking_record)
            contact_phone = contact.get('phone_number', '').lstrip('+')
            contact_name = contact.get('display_name', 'Guest')
        else:
            # Save without contact_id (will need to link later)
            print("   ⚠️ No contact_id found, booking saved without contact link")
            contact_phone = None
            contact_name = 'Guest'
        
        # Build summary message (this returns an INTERACTIVE message with buttons)
        summary_message = build_booking_summary(
            booking_ref, booking_date, adults, children, 
            parking_needed, activities, costs, 
            {'display_name': contact_name, 'phone_number': contact_phone}
        )
        
        # Send the summary via WhatsApp - USE send_interactive_message, NOT send_message!
        wa = adapters['whatsapp']
        
        if contact_phone:
            print(f"   📤 Sending booking summary to {contact_phone}")
            # THIS IS THE FIX - use send_interactive_message
            result = wa.send_interactive_message(contact_phone, summary_message)
            print(f"   Send result: {result}")
            
            if result.get('success'):
                # Save the confirmation message to database
                if contact.get('id'):
                    save_message(contact['id'], 'whatsapp', 'outgoing', f"Booking summary sent: {booking_ref}")
            else:
                print(f"   ❌ Failed to send summary: {result.get('error')}")
                # Fallback: send as text
                fallback_text = f"""📋 *ACTIVITY BOOKING SUMMARY*

Booking Reference: {booking_ref}
Total: ${costs['total']:.2f}

Reply with CONFIRM to proceed or CANCEL to decline."""
                wa.send_message(contact_phone, fallback_text)
        else:
            print(f"   ⚠️ No phone number found, cannot send summary")
